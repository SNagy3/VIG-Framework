...
def find_outer_turning_radius(M: float, g: float, C: float, nscan: int = 14000) -> float:
    r_plus = horizon_radius_bardeen(M, g)
...
def y_to_x(y: float, beta: float) -> float:
    return math.sqrt(max(y**2 - beta**2, 0.0))


def bardeen_outer_horizon_dimless(beta: float, nscan: int = 40000) -> tuple[float, float]:
    if abs(beta) < 1e-15:
        return 2.0, 2.0

    beta_c = 4.0 / (3.0 * math.sqrt(3.0))
    if beta >= beta_c:
        raise ValueError("Bardeen outer horizon requires beta < beta_c.")
...
def bardeen_critical_root_dimless(beta: float, nscan: int = 60000) -> tuple[float, float]:
    if abs(beta) < 1e-15:
        return 1.5, 1.5
...
# Dense edge scan
# ============================================================
def build_beta_grid(beta_c: float, beta_max_frac: float = 0.985) -> list[float]:
    """
    Dense grid reaching close to extremality.
    """
    beta_max = beta_max_frac * beta_c

    coarse = np.arange(0.00, min(beta_max, 0.50) + 1e-12, 0.05)
    mid = np.arange(0.52, min(beta_max, 0.70) + 1e-12, 0.02)
    fine = np.arange(0.705, beta_max + 1e-12, 0.005)

    beta = np.unique(np.concatenate([coarse, mid, fine]))
    beta = beta[beta < beta_c]
    if len(beta) == 0 or beta[0] != 0.0:
        beta = np.insert(beta, 0, 0.0)
    return [float(x) for x in beta]


def run_bardeen_edge_scan(
    outdir: str | Path = ".",
    M: float = 10.0,
    beta_grid: list[float] | None = None,
    beta_max_frac: float = 0.985,
    nC: int = 60,
    dmin: float = 1e-6,
    dmax: float = 1e-1,
    npts: int = 12000,
    ...
    if beta_grid is None:
        beta_grid = build_beta_grid(beta_c=beta_c, beta_max_frac=beta_max_frac)
    ...
    for beta in beta_grid:
        ...
        rows.append({
            ...
            "beta_c": beta_c,
            "g_c": beta_c * M,
            "beta_over_beta_c": float(beta / beta_c),
    ...
    master_csv = outdir / "vig_bardeen_edge_master_summary.csv"
    atlas_csv = outdir / "vig_bardeen_edge_atlas.csv"
    ...
    if make_plots:
        try:
            ...
            plt.title(r"Bardeen edge atlas: $x_*(\beta)$ and $x_+(\beta)$")
            plt.legend()
            plt.tight_layout()
            p = outdir / "vig_bardeen_edge_xstar_xplus.png"
            ...
            plt.title(r"Bardeen edge atlas: $\mathcal{B}(\beta)$")
            plt.tight_layout()
            p = outdir / "vig_bardeen_edge_B_beta.png"
            plt.savefig(p, dpi=200)
            plt.close()
            plot_files.append(str(p))
            ...
            plt.title(r"Bardeen edge atlas: normalized critical growth law")
            plt.tight_layout()
            p = outdir / "vig_bardeen_edge_Ccrit_over_CR.png"
            plt.savefig(p, dpi=200)
            plt.close()
            plot_files.append(str(p))
            ...
            plt.title(r"Bardeen edge scan: exact law vs asymptotic formulas")
            plt.legend()
            plt.tight_layout()
            p = outdir / "vig_bardeen_edge_exact_vs_asymptotics.png"
            plt.savefig(p, dpi=200)
            plt.close()
            plot_files.append(str(p))
            ...
            plt.title(r"Bardeen edge scan: critical growth law vs tail mean")
            plt.legend()
            plt.tight_layout()
            p = outdir / "vig_bardeen_edge_ccrit_vs_tail.png"
            plt.savefig(p, dpi=200)
            plt.close()
            plot_files.append(str(p))
        except Exception:
...
if __name__ == "__main__":
    result = run_bardeen_edge_scan(outdir=".", M=10.0, make_plots=True)
    print("saved:")
    ...
    print(f"beta_c = {result['beta_c']:.12f}")
    print(f"g_c    = {result['g_c']:.12f}")
